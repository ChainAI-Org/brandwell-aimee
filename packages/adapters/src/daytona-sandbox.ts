...
export function managedDesktopBrandingCommand(): string {
  return [
    ...
    '  wallpaper_profile="/tmp/aimee-wallpaper-render-$$"',
    '  timeout 25s google-chrome --headless=new --no-sandbox --disable-gpu --hide-scrollbars --user-data-dir="$wallpaper_profile" --window-size=1440,900 --screenshot="$wallpaper_png" "file://$wallpaper_svg" >/dev/null 2>&1 || true',
    '  rm -rf -- "$wallpaper_profile"',
    "fi",
    'if ! test -s "$wallpaper_png" && command -v rsvg-convert >/dev/null 2>&1; then',
    '  rsvg-convert -w 1440 -h 900 "$wallpaper_svg" -o "$wallpaper_png" >/dev/null 2>&1 || true',
    "fi",
    'if test -s "$wallpaper_png" && command -v identify >/dev/null 2>&1; then',
    '  wallpaper_colors="$(identify -format "%k" "$wallpaper_png" 2>/dev/null || true)"',
    '  case "$wallpaper_colors" in ""|*[!0-9]*) rm -f -- "$wallpaper_png" ;; *) test "$wallpaper_colors" -gt 8 || rm -f -- "$wallpaper_png" ;; esac',
    "fi",
    ...
    "  xfconf-query -c xfce4-desktop -p /desktop-icons/style -s 0 >/dev/null 2>&1 || xfconf-query -c xfce4-desktop -p /desktop-icons/style -n -t int -s 0 >/dev/null 2>&1 || true",
    "  for icon_key in show-home show-filesystem show-trash show-removable; do",
    '    key="/desktop-icons/file-icons/$icon_key"',
    '    xfconf-query -c xfce4-desktop -p "$key" -s false >/dev/null 2>&1 || xfconf-query -c xfce4-desktop -p "$key" -n -t bool -s false >/dev/null 2>&1 || true',
    "  done",
