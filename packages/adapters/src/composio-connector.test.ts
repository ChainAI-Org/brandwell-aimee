...
describe("composio tool mapping", () => {
  ...
  it("only fetches live Composio slugs when a AIMEE row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but AIMEE is still pending", () => {
    expect(
      planLiveConnectionSync(
        [
