...
const APP_MARKER = "Existing AIMEE instance ready";
...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.end(
      `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>AIMEE</title></head><body><main>${APP_MARKER}</main></body></html>`,
    );
  });
...
test("connecting to an existing instance verifies, saves, and opens it", async () => {
  ...
  await expect(setup.locator("#status")).toHaveText(`AIMEE answered at ${serverUrl}.`);
...
test("a session-pending shell skeleton is not accepted as a ready app", async () => {
  const skeletonHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>AIMEE</title></head>
...
test("a post-session ready app mount is accepted", async () => {
  const readyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>AIMEE</title>
...
test("a session-ready marker without a route surface is not accepted", async () => {
  const emptyReadyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>AIMEE</title></head>
...
test("a generic web page is not accepted as an AIMEE server", async () => {
  const plain = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end("<!doctype html><p>not AIMEE</p>");
  });
