...
describe("marketing site middleware", () => {
  it("negotiates Markdown on the canonical page URL", async () => {
    const response = middleware(
      new Request("https://aimee.brandwell.ai/", {
    ...
    await expect(response.text()).resolves.toContain("# BrandWell's AIMEE");
  });

  it("gives generic agent fetches a recoverable Markdown 404", async () => {
    const response = middleware(
      new Request("https://aimee.brandwell.ai/does-not-exist", {
        headers: { accept: "*/*" },
      }),
    );
    ...
    await expect(response.text()).resolves.toContain(
      "https://aimee.brandwell.ai/sitemap-index.xml",
    );
  });
